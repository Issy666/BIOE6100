...
class tactile_receptors():
    def __init__(self,Ttype=' ',Pls=0): #whether load fitting parameters 1:Yes
        ...
        self.VL=15*1e-3  #v
        paras_dict=np.load('Data/fitting_paras.npy', allow_pickle=True).item()
        if(self.Ttype=="SA1"):
            ...
            self.maxfr=300
        self.Mt=np.asmatrix([[np.cos(-self.thea),np.sin(-self.thea)],[-np.sin(-self.thea),np.cos(-self.thea)]]);

    """
    Set Population: is a function that ....
    Inputs
    punit_buf: For Mechanorecpeor PA, SA1, FA1 in the loc_pos_buf_fingertip data file -> locations of that receptor in (fingerprint, palm, bfinger)
    grid_buf:
    simTime:
    sample_rate:
    Density:
    ...
    def set_population(self,punit_buf,grid_buf,simTime=1,sample_rate=1000,Density=150,youngs=50*1e3,roi=[]):  
        ...
        self.G=np.asmatrix(np.zeros((self.Rm*self.Rn,self.Rm*self.Rn)))
        self.Gi=np.asmatrix(np.zeros((self.Rm*self.Rn,self.Rm*self.Rn))) 
    ...
    def points_mapping_entrys(self,stimuli):
        ...
        for i in range(0,len(stimuli)):
            ...
            entrys[i]=np.where(dists==dists.min())[0][0]
    ...
    def population_simulate(self,EEQS=[],Ips=[[],'Pressure'],acquire_spikes=True,disinf=True,noise=10): #Ips: Inputing parameters
        ...
        for pt in range(2,self.t.size):  #时间 t.size
            ...
            if(Ips[1]=="Pressure"):
                denom=self.Nds*Dbp*1e-3*np.sqrt(RI[RI>0].size/4)
                Dp=0.0 if denom==0 else Cm*Ips[0][pt,2]/denom
            ...
            self.Uc[:,pt:pt+1]=self.Gi*np.asmatrix(self.Is[:,pt:pt+1])  
            # Single-unit model
            tmp=0
...
def isPoisWithinPoly(poly,pois):
    ...
    for i in range(len(poly)-1): #[0,len-1]
        out=np.bool_(np.zeros(len(pois)))
        s_poi=poly[i,:]
        e_poi=poly[i+1,:]
...
def points_map_to_entrys(stimuli,thea,grid_buf,Density):
    ...
    entry_buf=np.int16(tmp[:,0]+tmp[:,1]*(100*2))
    v_pos=entry_buf;
    Mt=np.asmatrix([[np.cos(thea),np.sin(thea)],[-np.sin(thea),np.cos(thea)]]);
...
def Mt(thea):
    return np.asmatrix([[np.cos(thea),np.sin(thea)],[-np.sin(thea),np.cos(thea)]])
...
def band_filter(dt,t,fl,fh,signal):
    ...
    for time in range(0,t.size-1):  #时间 t.size
        ...
        s1[time+1] =(1-wbl*dt)*s1[timme]+ (s0[time+1]-s0[time])
    return s1 
...
def linear_filter(stimulus, rf_width, dt):
    # Gaussian receptive field kernel
    t = np.arange(-3*rf_width, 3*rf_width, dt)
    kernel = np.exp(-t**2 / (2*rf_width**2))
    kernel /= kernel.sum()  # normalise
    return np.convolve(stimulus, kernel, mode='same')

# Rectified linear (simplest)
def nonlinearity_relu(x, gain=50):
    return gain * np.maximum(0, x)

# Softplus (smoother, more realistic)
def nonlinearity_softplus(x, gain=50, beta=5):
    return gain * np.log1p(np.exp(beta * x)) / beta

# Sigmoid (saturating)
def nonlinearity_sigmoid(x, gain=100, threshold=0.5):
    return gain / (1 + np.exp(-10*(x - threshold)))


def poisson_spikes(firing_rate, dt):
    # firing_rate in Hz, dt in seconds
    prob = firing_rate * dt
    return np.random.poisson(prob)  # spikes per bin

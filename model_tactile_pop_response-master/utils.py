...
utils.py - small helpers used by the Ouyang/Saal reproduction scripts:
    statistical tests, curve fits, RMSE / R², and a few text-file readers.

Inherited from the original Ouyang 2019 codebase; comments and function
signatures preserved so the Figs_from_Ouyang/ scripts still work unchanged.
"""
import math
import numpy as np
...
def wilcoxon_signed_rank_test(y1, y2):
    """Paired non-parametric test (Wilcoxon signed-rank). Prints the result."""
    res = stats.wilcoxon(y1, y2)
    print(res)


def wilcoxon_rank_sum_test(x, y):
    """Two-sample non-parametric test (Mann-Whitney U). Prints the result."""
    res = stats.mannwhitneyu(x, y)
    print(res)


def rmse(y_test, y):
    """Root mean square error between prediction `y_test` and ground truth `y`."""
    return np.sqrt(np.mean((y_test - y) ** 2))


def R2(y_test, y_true):
    """Coefficient of determination, 1 − RSS / TSS. 1 = perfect, 0 = baseline."""
